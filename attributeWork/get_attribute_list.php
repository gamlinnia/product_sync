...
function main() {
    ...
    switch ($mode) {
        case '4' :
            ...
            $prompt = promptMessageForInput('add ignore list ?(Y/n)', array('y', 'Y', 'n', 'N'));
            if($prompt == 'y') {
            ...
            if (empty($new_attr_label)) {
                ...
                foreach($attr_collection as $_attr) {
                    $prompt = promptMessageForInput('sure to delete attribute: ' . $attr->getData('attribute_code') . ' (Y/n)?', array('y', 'Y', 'n', 'N'));
                    if($prompt == 'y') {
            ...
            if (count($toAddArray) > 0) {
                $prompt = strtolower(promptMessageForInput('add below options ?(Y/n) or modify these options(M/m): ' . implode(' | ', $toAddArray) , array('y', 'Y', 'n', 'N', 'm', 'M')));
...
//                    Zend_Debug::dump($newOptionsArray);
                    $prompt = strtolower(promptMessageForInput('sure to add these new options above ?(Y/n)', array('y', 'Y', 'n', 'N')));
                    if($prompt =='y') {
...
//                    ));
                ...
                $prompt = strtolower(promptMessageForInput('delete ' . $old_attr_code . ' (Y/n) ?', array('y', 'Y', 'n', 'N')));
                if($prompt == 'y') {
                    Mage::getModel('eav/entity_attribute')->load($_attr->getId())->delete();
                }
                sleep(3);
